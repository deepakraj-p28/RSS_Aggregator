...
const getUsersForFeed = `-- name: GetUsersForFeed :many
SELECT id, created_at, updated_at, user_id, feed_id FROM feed_follows WHERE feed_id = $1
`

func (q *Queries) GetUsersForFeed(ctx context.Context, feedID uuid.UUID) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, getUsersForFeed, feedID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
